    ...
    */
    ...
    public static function getMessagesAdminList()
    {
        ...
        while($row = $result->fetch()) {
            ...
            $to_user_name = self::getUsername($to_user_id);
            $chats[$i]['chat_message_id'] = $row['chat_message_id'];
            $chats[$i]['to_user_id'] = $row['to_user_id'];
...
public static function chatHistory($from_user_id, $to_user_id)
{
     $db = Db::getConnection();
    $query = "
...
public static function chatAdminHistory($from_user_id, $to_user_id)
{
     $db = Db::getConnection();
    $query = "
 SELECT * FROM chat_message 
...
public static function sendMessage($send)
    {
        ...
        // Получение и возврат результатов.(Подгтовительный запрос)
        $result = $db->prepare($sql);
        $result->bindParam(':to_user_id', $send['to_user_id'], PDO::PARAM_INT);
